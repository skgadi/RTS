...
\begin{tikzpicture}
	\begin{axis}[domain=-1:1, tick style={draw=none}, ticks=none, ymax=1.1, ymin=-1.1, xmax=1.1, xmin=-1.1, line width=3, width=5cm, height=3cm]
		\node[] at (axis cs: 0, 0.0) {\Huge $\sum_{i=0}^{n}[u_i]$};
	\end{axis}
\end{tikzpicture}

\begin{tikzpicture}
	\begin{axis}[domain=-1:1, tick style={draw=none}, ticks=none, ymax=1.1, ymin=-1.1, xmax=1.1, xmin=-1.1, line width=3, width=5cm, height=3cm]
		\node[] at (axis cs: 0, 0.0) {\Huge $\prod_{i=0}^{n}[u_i]$};
	\end{axis}
\end{tikzpicture}
